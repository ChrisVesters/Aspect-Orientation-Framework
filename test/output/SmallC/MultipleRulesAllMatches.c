...
int _a_myPow_(int a, int b) {
	int result = 0;
	printf("BEGIN A!\n");
	{
		_c();
		result = myPow_exec_(a, b);
	}
...
int myPow_call_(int a, int b) {
	int _result_;
	_result_ = _a_myPow_(a, b);
	return _result_;
}

int _a_mySqrt_(int a) {
	int result = 0;
	printf("BEGIN A!\n");
	{
		_c();
		result = mySqrt_exec_(a);
	}
