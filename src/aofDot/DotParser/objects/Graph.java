...
package aofDot.DotParser.objects;
...
public class Graph {
	 ...
	 * @see aofDot.DotParser.objects.Graph#DIRECTED
	 * @see aofDot.DotParser.objects.Graph#UNDIRECTED
	 */
	public int getType() {
		return type;
	}
		...
		if(attributes.size()>0) { 		
			...
			while(e.hasMoreElements()) {
				...
				if(!attributes.get(k).equals("")) {
					r.append("=");
					if ((-1 == attributes.get(k).indexOf(" ")) && (-1 == attributes.get(k).indexOf(","))) {
						r.append(attributes.get(k));
					} else if (-1 != attributes.get(k).indexOf("\"")) {
						r.append(attributes.get(k));
					} else {
						r.append("\"");
						r.append(attributes.get(k));
